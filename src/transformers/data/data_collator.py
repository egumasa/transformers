...
def tf_default_data_collator(features: List[InputDataClass]) -> Dict[str, Any]:
    ...
    if label_col_name is not None:
        if isinstance(first[label_col_name], tf.Tensor):
            dtype = tf.int64 if first[label_col_name].dtype.is_integer(
            ) else tf.float32
        elif isinstance(first[label_col_name], np.ndarray) or isinstance(
                first[label_col_name], np.generic):
            dtype = tf.int64 if np.issubdtype(first[label_col_name].dtype,
                                              np.integer) else tf.float32
        elif isinstance(first[label_col_name], (tuple, list)):
            dtype = tf.int64 if isinstance(first[label_col_name][0],
                                           int) else tf.float32
        else:
            dtype = tf.int64 if isinstance(first[label_col_name],
                                           int) else tf.float32
    ...
    for k, v in first.items():
        if k not in ("label", "label_ids",
                     "labels") and v is not None and not isinstance(v, str):
...
def _torch_collate_batch(examples,
    ...
    if are_tensors_same_length and (pad_to_multiple_of is None or
...
def _tf_collate_batch(examples,
                      tokenizer,
                      pad_to_multiple_of: Optional[int] = None):
    import numpy as np
    import tensorflow as tf
...
def _numpy_collate_batch(examples,
    ...
    if pad_to_multiple_of is not None and (max_length % pad_to_multiple_of !=
        ...
        max_length = (
            (max_length // pad_to_multiple_of) + 1) * pad_to_multiple_of
    result = np.full(shape=(len(examples), max_length),
...
def tolist(x):
    if isinstance(x, list):
        return x
                 ...
                 "numpy"):  # Checks for TF tensors without needing the import
        x = x.numpy()
    return x.tolist()
...
class DataCollatorForLanguageModeling(DataCollatorMixin):
    ...
    def __post_init__(self):
        ...
        if self.tf_experimental_compile:
            ...
            self.tf_mask_tokens = tf.function(self.tf_mask_tokens,
                                              jit_compile=True)
    ...
    def tf_bernoulli(shape, probability):
        ...
        return tf.cast(prob_matrix - tf.random.uniform(shape, 0, 1) >= 0,
                       tf.bool)
    ...
    def tf_mask_tokens(
        ...
        masked_indices = self.tf_bernoulli(
            input_shape, self.mlm_probability) & ~special_tokens_mask
        ...
        indices_random = self.tf_bernoulli(
            input_shape, 0.1) & masked_indices & ~indices_replaced
        random_words = tf.random.uniform(input_shape,
                                         maxval=vocab_size,
                                         dtype=tf.int64)
    ...
    def tf_call(
        ...
        if self.mlm:
            ...
            if self.tokenizer.pad_token_id is not None:
                ...
                labels = tf.where(labels == self.tokenizer.pad_token_id, -100,
                                  labels)
            else:
    ...
    def numpy_mask_tokens(
        ...
        masked_indices = np.random.binomial(
            ...
            size=probability_matrix.shape).astype(np.bool)
        labels[~masked_indices] = -100  # We only compute loss on masked tokens

        # 80% of the time, we replace masked input tokens with tokenizer.mask_token ([MASK])
        indices_replaced = np.random.binomial(
            1, 0.8, size=labels.shape).astype(np.bool) & masked_indices
        inputs[indices_replaced] = self.tokenizer.mask_token_id

        # 10% of the time, we replace masked input tokens with random word
        # indices_random = torch.bernoulli(torch.full(labels.shape, 0.5)).bool() & masked_indices & ~indices_replaced
        indices_random = (np.random.binomial(1, 0.5, size=labels.shape).astype(
            np.bool) & masked_indices & ~indices_replaced)
        random_words = np.random.randint(low=0,
...
class DataCollatorForWholeWordMask(DataCollatorForLanguageModeling):
    ...
    def _whole_word_mask(self, input_tokens: List[str], max_predictions=512):
        ...
        cand_indexes = []
        special_tokens = [
            val for key, val in self.tokenizer.special_tokens_map.items()
            if key not in ['unk_token', 'mask_token']
        ]
        is_bert_tokenizer = isinstance(self.tokenizer,
    ...
    def tf_mask_tokens(self, inputs: Any, mask_labels: Any) -> Tuple[Any, Any]:
        ...
        masked_indices = masked_indices & ~tf.cast(special_tokens_mask,
                                                   dtype=tf.bool)
        if self.tokenizer._pad_token is not None:
        ...
        inputs = tf.where(indices_replaced, self.tokenizer.mask_token_id,
                          inputs)
    ...
    def numpy_mask_tokens(self, inputs: Any,
                          mask_labels: Any) -> Tuple[Any, Any]:
        """
        Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original. Set
        'mask_labels' means we use whole word mask (wwm), we directly mask idxs according to it's ref.
...
class DataCollatorForSOP(DataCollatorForLanguageModeling):
    ...
    def mask_tokens(self, inputs: Any) -> Tuple[Any, Any, Any]:
        ...
        labels[
            ~masked_indices] = -100  # We only compute loss on masked tokens, -100 is default for CE compute
...
class DataCollatorForPermutationLanguageModeling(DataCollatorMixin):
    ...
    def torch_mask_tokens(self, inputs: Any) -> Tuple[Any, Any, Any, Any]:
        ...
        for i in range(labels.size(0)):
            ...
            while cur_len < max_len:
                ...
                span_length = torch.randint(1, self.max_span_length + 1,
                                            (1, )).item()
                # Reserve a context of length `context_length = span_length / plm_probability` to surround the span to be masked
                context_length = int(span_length / self.plm_probability)
                # Sample a starting point `start_index` from the interval `[cur_len, cur_len + context_length - span_length]` and mask tokens `start_index:start_index + span_length`
                start_index = cur_len + torch.randint(
                    context_length - span_length + 1, (1, )).item()
    ...
    def tf_mask_tokens(self, inputs: Any) -> Tuple[Any, Any, Any, Any]:
        ...
        masked_indices = tf.cast(tf.convert_to_tensor(masked_indices),
                                 dtype=tf.bool)
        ...
        labels = tf.where(masked_indices, labels,
        ...
        for i in range(len(labels)):
            ...
            perm_index = tf.random.shuffle(
                perm_index)  # Shuffles along the first dimension
            # Flatten this out into the desired permuted factorisation order
            ...
            # (1) They can be seen by all other positions
            # (2) They cannot see masked positions, so there won't be information leak
            perm_index = tf.where(~masked_indices[i] & non_func_mask[i], -1,
                                  perm_index)
            # The logic for whether the i-th token can attend on the j-th token based on the factorisation order:
            # 0 (can attend): If perm_index[i] > perm_index[j] or j is neither masked nor a functional token
            # 1 (cannot attend): If perm_index[i] <= perm_index[j] and j is either masked or a functional token
            perm_mask.append((tf.reshape(perm_index, (labels_shape[1], 1)) <=
                              tf.reshape(perm_index, (1, labels_shape[1])))
                             & masked_indices[i])
        perm_mask = tf.stack(perm_mask, axis=0)

        return tf.cast(inputs, tf.int64), tf.cast(
            perm_mask, tf.float32), target_mapping, tf.cast(labels, tf.int64)
    ...
    def numpy_mask_tokens(self, inputs: Any) -> Tuple[Any, Any, Any, Any]:
        ...
        for i in range(labels.shape[0]):
            ...
            # 1 (cannot attend): If perm_index[i] <= perm_index[j] and j is either masked or a functional token
            perm_mask[i] = (perm_index.reshape(
                (labels.shape[1], 1)) <= perm_index.reshape(
                    (1, labels.shape[1]))) & masked_indices[i]

        return inputs.astype(
            np.int64), perm_mask, target_mapping, labels.astype(np.int64)
